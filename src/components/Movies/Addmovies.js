...
const Addmovies = () => {
    ...
    const handleSubmit = (e) => {
        e.preventDefault()   
        addmovie({...inputs,movies}).then((res)=>console.log(res)).catch((err)=>console.log(err))
        ...
        <div>
            <form onSubmit={handleSubmit}>
                <Box width="50%" padding={10} margin="auto" display='flex' flexDirection={'Column'} boxShadow={"10px 10px 20px #ccc"}>
                    ...
                    <Button type="submit" value={inputs} variant="contained" sx={{
                        width: "30%", margin: "auto", bgcolor: "#2b2d42", ":hover": {
                            bgcolor: "#777"
                        }
                    }}>Add movie</Button>
                </Box>
